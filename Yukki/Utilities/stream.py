...
async def start_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        await mystic.delete()
        cap = f"🎥<b>__Oynatılan Parça:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n💡<b>__Bilgi:__</b> [Ek Bilgi Alın](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n👤**__Talep Eden:__** {CallbackQuery.from_user.mention}"
        final_output = await CallbackQuery.message.reply_photo(
            photo=thumb,
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            ...
            caption=(
                f"🎬<b>__Müzik:__ </b> [Telegram Üzerinden Ses Verildi]({link})\n⏳<b>__Süre:__</b> {duration_min} \n👤<b>__Talep Eden:__ </b>{message.from_user.mention} \n🚧<b>__Alındığı Sıra:__</b> <b>#{position}!</b>"
            ),
        ...
        await mystic.delete()
        cap = f"🎥<b>__Oynatılan Parça:__ </b>[Telegram Üzerinden Ses Verildi]({link})\n👤**__Talep Eden:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
