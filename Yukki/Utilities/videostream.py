...
async def start_stream_video(message, file, title, mystic):
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            ...
            caption=(
                f"🎬<b>__Video:__ </b> [Telegram Yoluyla Verilen Video]({link})\n\n👤<b>__Talep Eden:__ </b>{message.from_user.mention} \n🚧<b>__Alınan Sıra:__</b> <b>#{position}!</b>"
            ),
        ...
        await mystic.delete()
        cap = f"🎥<b>__Oynatılan Parça:__ </b>[Telegram Yoluyla Verilen Video]({link})\n👤**__Talep Eden:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_video_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        final_output = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=(
                f"🎬<b>Oynatılan Video:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n⏳<b>__Süre:__</b> {duration_min} \n💡<b>__Bilgi:__</b> [Ek Bilgi Alın](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n👤<b>__Talep Eden:__ </b>{CallbackQuery.from_user.mention} \n🚧<b>__ Alınan Video Sırası:__</b> <b>#{position}!</b>"
            ),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        await CallbackQuery.message.delete()
